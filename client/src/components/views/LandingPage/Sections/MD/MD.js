import React from 'react'
import './MD.css'

function MD() {
    return (
        <div>
            <section className="MD">
                <div className="MDInner">
                    <article className="MD4">
                        <div className="MD4_tit"><h2>MD 추천상품</h2></div>
                        <div className="MD4-1">
                            <img src="/img/main/md4-1.png" alt="안국 루테인지아잔틴 포뮬라 10박스"/>
                            <div className="MD4_txt">
                                <h3>안국 루테인지아잔틴 포뮬라 10박스</h3>
                                <span>NS 홈쇼핑 첫 런칭 후 매진 구성!<br/>AGmall 단독으로 소개합니다.</span>
                            </div>
                        </div>
                        <div className="MD4-2">
                            <img src="/img/main/md4-2.png" alt="코박사에스"/>
                            <div className="MD4_txt">
                                <h3>코박사에스</h3>
                                <span>리뉴얼 업그레이드 되어 돌아왔어요.<br/>코건강, 정상적인 면역기능, 뼈건강을 한번에 챙겨요.</span>
                            </div>
                        </div>
                        <div className="MD4-3">
                            <img src="/img/main/md4-3.png" alt="인생뷰티 시네트롤 브이세븐"/>
                            <div className="MD4_txt">
                                <h3>인생뷰티 시네트롤 브이세븐</h3>
                                <span>국내에서 최초로 선보이는 프랑스산 다이어트 최신소재<br/>'시네트롤'로 시작하세요.</span>
                            </div>
                        </div>
                        <div className="MD4-4">
                            <img src="/img/main/md4-4.png" alt="인생뷰티 저분자 콜라겐 유산균업"/>
                            <div className="MD4_txt">
                                <h3>인생뷰티 저분자 콜라겐 유산균업</h3>
                                <span>푸석푸석 생기없는 내 모습이 걱정될 때<br/>화장실을 다녀와도 개운하지 않을 때 필요한 순간입니다.</span>
                            </div>
                        </div>
                    </article>
                    <article className="MDList">
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist1.png" alt="아이세이프 루테인"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist2.png" alt="눈에좋은 루테인 플러스"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist3.png" alt="루테인 미니100"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist4.png" alt="루테인 지아잔틴"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist5.png" alt="아이포커스"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist6.png" alt="오메가 루테인 플러스"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist7.png" alt="알티지오메가3"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist8.png" alt="밀크씨슬"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist9.png" alt="안심 비타민"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist10.png" alt="건강 선물세트"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist11.png" alt="인생뷰티"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist12.png" alt="저분자 콜라겐"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist13.png" alt="코박사"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist14.png" alt="코박사 키즈"/>
                        </a>
                        <a href="!#" onClick={e=>{e.preventDefault();}} title="" target="_self">
                            <img src="/img/main/MDlist15.png" alt="편강 청강원"/>
                        </a>
                    </article>
                </div>
            </section>
        </div>
    )
}

export default MD
